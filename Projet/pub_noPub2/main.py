...
def main():
    ...
    if model:
        ...
        if latest_acquisition:
            ...
            enregistrer_snapshot(adresse_ip, port, nom_utilisateur, mot_de_passe, duree_salve, nb_images)

            # Charger les images enregistrées à partir du dossier de sauvegarde
            snapshot_subfolder = os.path.join(snapshot_folder, date_du_jour)
            image_filenames = [os.path.join(snapshot_subfolder, f'snapshot_{i}.jpg') for i in range(nb_images)]
